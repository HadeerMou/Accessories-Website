...
import { createReview, deleteReview, listAllReviews } from "./reviews.service.js";
...
export async function removeReview(request: Request, response: Response, next: NextFunction) {
  try {
    const reviewId = request.params.reviewId;
    if (!reviewId) throw new HttpError(400, "Invalid review id");
