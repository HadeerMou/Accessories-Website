...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/products/${params.id}`, { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(
            body?.error ?? body?.detail ?? "Could not load product",
          );
      ...
      .catch((cause) => {
        if (cause.name !== "AbortError") {
          const mockProduct =
            findMockProductById(params.id) ??
            findMockProductById("mock-accessories-1");
          if (mockProduct) {
  ...
  const handleReviewSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(
        cause instanceof Error ? cause.message : "Could not submit review",
      );
    } finally {
    ...
    return (
      <main className="min-h-screen flex flex-col bg-[#FAF8F5]">
        ...
        <section className="mx-auto flex max-w-6xl flex-1 flex-col justify-center px-8 py-24">
          ...
          <Link
            href="/women"
            className="mt-4 inline-flex w-fit rounded-full bg-[#26372f] px-5 py-2.5 text-xs font-semibold uppercase tracking-wider text-white"
          >
            Back to shop
          </Link>
        </section>
    ...
    <main className="min-h-screen flex flex-col bg-[#FAF8F5]">
      ...
      <section className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-10 px-8 py-24 lg:flex-row lg:items-start">
        <div className="w-full lg:w-[45%]">
          <div className="aspect-[4/5] overflow-hidden rounded-[2rem] bg-[#e8e2d8]">
            {image ? (
              ...
              <div className="grid h-full place-items-center font-serif text-3xl text-black/20">
                Aura
              </div>
            )}
          </div>
        ...
        <div className="w-full lg:w-[55%]">
          <Link
            href="/women"
            className="mb-6 inline-flex items-center gap-2 text-sm uppercase tracking-widest text-[#2C2A28]/60"
          >
            <ArrowLeft size={16} />{" "}
            {locale === "ar" ? "العودة للتسوق" : "Back to shop"}
          </Link>
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-3xl font-serif text-[#2C2A28]">{name}</h1>
              <p className="mt-2 text-sm uppercase tracking-[0.3em] text-[#2C2A28]/45">
                {locale === "ar"
            ...
            <button
              ...
              className={`rounded-full p-3 ${wishlist ? "bg-[#26372f] text-white" : "border border-black/10 bg-white text-[#2C2A28]"}`}
            >
              <Heart size={18} fill={wishlist ? "currentColor" : "none"} />
            </button>
          </div>
          <p className="mt-6 text-sm leading-7 text-[#2C2A28]/70">
            {description ||
              (locale === "ar"
                ? "لا يوجد وصف متاح."
          ...
          <div className="mt-8 flex items-center gap-4">
            <div className="text-2xl font-semibold text-[#2C2A28]">
              ${price.toFixed(2)}
            </div>
          ...
          <button
            ...
            className="mt-8 flex items-center justify-center gap-2 rounded-full bg-[#26372f] px-6 py-3 text-sm font-semibold uppercase tracking-wider text-white"
          >
            <ShoppingBag size={16} />{" "}
            {locale === "ar" ? "أضف إلى السلة" : "Add to bag"}
          </button>
          {message ? (
            <p className="mt-4 text-sm text-[#26372f]">{message}</p>
          ) : null}
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-6xl px-8 py-16 border-t border-black/10">
        <h2 className="text-2xl font-serif text-[#2C2A28] mb-8">
          ...
          <div className="space-y-6">
            {!product.reviews || product.reviews.length === 0 ? (
              <p className="text-sm text-[#2C2A28]/60">
                {locale === "ar"
                  ? "لا توجد تقييمات حتى الآن. كن أول من يكتب تقييماً!"
                  ...
                  <div className="flex items-center gap-1 mb-3">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        size={14}
                        fill={star <= review.rating ? "#d1ae68" : "none"}
                        color={star <= review.rating ? "#d1ae68" : "#ccc"}
                      />
                    ))}
                  </div>
                  {review.comment && (
          ...
          <div className="bg-white p-8 rounded-2xl border border-black/5 shadow-sm h-fit">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-5">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Star
                        size={24}
                        fill={star <= reviewRating ? "#d1ae68" : "none"}
                        color={star <= reviewRating ? "#d1ae68" : "#ccc"}
                      />
                    </button>
                  ))}
                </div>
              ...
              <p className="text-[10px] text-center text-black/40 mt-4">
                {locale === "ar"
                  ? "يمكنك فقط تقييم المنتجات التي قمت بشرائها."
