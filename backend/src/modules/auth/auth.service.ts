...
export function parseToken(token: string): Claims { const [body, signature] = token.split("."); if (!body || !signature) throw new HttpError(401, "Invalid access token"); const expected = sign(body); if (signature.length !== expected.length || !timingSafeEqual(Buffer.from(signature), Buffer.from(expected))) throw new HttpError(401, "Invalid access token"); let claims: Claims; try { claims = JSON.parse(Buffer.from(body, "base64url").toString()) as Claims; } catch { throw new HttpError(401, "Invalid access token"); } if (!claims.sub || claims.exp < Date.now() / 1000) throw new HttpError(401, "Access token expired"); return claims; }
